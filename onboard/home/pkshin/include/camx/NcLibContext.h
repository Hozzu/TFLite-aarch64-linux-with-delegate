...
enum EICAArraySizes{
    ...
    ICA_V30_GRID_TRANSFORM_LORES_WIDTH = 35,        /**< ICA v30 low resolution grid width */
    ICA_V30_GRID_TRANSFORM_LORES_HEIGHT = 27,       /**< ICA v30 low resolution grid height */
    ICA_V30_GRID_TRANSFORM_HIRES_WIDTH = 67,        /**< ICA v30 high resolution grid width */
    ICA_V30_GRID_TRANSFORM_HIRES_HEIGHT = 51        /**< ICA v30 high resolution grid height */
};
...
// RTODO: remove LmcParameters once FW LMC struct is integrated

#define FW_LMC_STRUCT_MISSING

#ifdef FW_LMC_STRUCT_MISSING

typedef struct _LmcPassParameters
{
    uint32_t lmIrregMantissa;  //One of the parameters that helps calculate the irregularity threshold
    int32_t  lmIrregExp;  //One of the parameters that helps calculate the irregularity threshold
    uint32_t lmConfMantisaa;  //One of the parameters that helps calculate confidence threshold
    int32_t  lmConfExp;  //One of the parameters that helps calculate confidence threshold
    uint32_t lmSadC2;   //One of the parameters that helps calculate SAD threshold in the im_sad_0 check
    uint32_t lmSadC1;   //One of the parameters that helps calculate SAD threshold in the im_sad_0 check
    uint32_t lmDefaultSadC2; //One of the parameters that helps calculate SAD threshold in the im_sad_0 check
    uint32_t lmDefaultSadC1; //One of the parameters that helps calculate SAD threshold in the im_sad_0 check
    uint32_t lmMaxY; //Maximum value for motion vector in y direction
    uint32_t lmMaxX; //Maximum value for motion vector in x direction
    uint32_t lmSmoothnessA; //One of the parameters that helps calculate the smoothness factor
    uint32_t lmSmoothnessM; //One of the parameters that helps calculate the smoothness factor
    uint32_t ctc_lmc_mv_diff_thr; //Motion vector difference threshold for LMC in ICA
    uint32_t lmEnable; // NOTE: currently this is not in FW struct, but should be added
}LmcPassParameters;

typedef struct _LmcParameters
{
    uint32_t enableLmcInIca; // NOTE: currently this is not in FW struct but master enable is. This should be changed is here.
