...
#ifndef STRIPE_MIN
#define  STRIPE_MIN( x, y ) ( ((x) < (y)) ? (x) : (y) )
#endif

#ifndef STRIPE_MAX
#define  STRIPE_MAX( x, y ) ( ((x) > (y)) ? (x) : (y) )
#endif

#ifndef STRIPE_ABS
#define  STRIPE_ABS( x ) ( ((x) > (0)) ? (x) : (-x) )
#endif
...
#ifndef BF_STATS_RGNCOUNT_V25
#define BF_STATS_RGNCOUNT_V25 181
#endif

#ifndef CSIDPE_V10_MAX_BLOCK_HEIGHT
#define CSIDPE_V10_MAX_BLOCK_HEIGHT (64)
#endif

#ifndef CSIDPE_V10_MAX_BLOCK_WIDTH
#define CSIDPE_V10_MAX_BLOCK_WIDTH (64)
#endif

#ifndef MIN_FIR_PADDING_PDAF
#define MIN_FIR_PADDING_PDAF (2)
#endif

#ifndef MAX_PIXEL_ENTRIES_RS
#define MAX_PIXEL_ENTRIES_RS (32*4)
#endif

#ifndef MIN_SAD_PIXELS
#define MIN_SAD_PIXELS (16)
#endif

#ifndef PDAF_PHASE
#define PDAF_PHASE (32)
#endif

#ifndef PDAF_V20_ROLLOFF_MESH_POINTS_H
#define PDAF_V20_ROLLOFF_MESH_POINTS_H (17)
#endif

#ifndef PDAF_V20_ROLLOFF_MESH_POINTS_V
#define PDAF_V20_ROLLOFF_MESH_POINTS_V (13)
#endif
...
typedef struct
{
    uint16_t lut_bank_sel;
    uint16_t firstPixelPre;
    uint16_t lastPixelPre;
    uint16_t firstPixelPost;
    uint16_t lastPixelPost;
    uint16_t csidHorizScale;
    uint16_t rgnHOffset;
    uint16_t rgnWidth;
    uint16_t rgnHNum;
    uint16_t nativeBlockWidth;
    uint16_t nativeBlockHeight;
    uint16_t CSID_pe_map[CSIDPE_V10_MAX_BLOCK_HEIGHT][CSIDPE_V10_MAX_BLOCK_WIDTH];
    uint16_t RSpixelLUT[MAX_PIXEL_ENTRIES_RS];
    uint16_t RSinWidth;
    uint16_t RSoutWidth;
    uint16_t RSinHeight;
    uint16_t RSoutHeight;
    uint16_t PSoutWidth;
    uint16_t lineExtractorLength;
    uint32_t hPhaseInit;
    uint16_t pdafMode;
    uint32_t SADPhaseMask0;
    uint32_t SADPhaseMask1;
    uint16_t HBinPixelNum;
    uint16_t IIRPadding;
    uint16_t IIRFilterEnable;
    uint16_t gainMapEnable;
    uint16_t meshPointsHNum;
    uint16_t meshPointsVNum;
    uint16_t HDREnable;
    uint32_t hPhaseStep;
    uint16_t GainMapLUTL[PDAF_V20_ROLLOFF_MESH_POINTS_H * PDAF_V20_ROLLOFF_MESH_POINTS_V];
    uint16_t GainMapLUTR[PDAF_V20_ROLLOFF_MESH_POINTS_H * PDAF_V20_ROLLOFF_MESH_POINTS_V];
}
PDAFv20InputParam;
...
typedef struct
{
    ...
    uint16_t output_l;
    int16_t  preload;
    uint16_t dithering_mode_y;
    uint16_t dithering_mode_c;
    uint32_t v_rounding_mode;
    uint32_t h_rounding_mode;
    int16_t dummy_field_for_byte_padding;
    uint32_t phase_init;
    uint32_t phase_step;
}
...
typedef struct
{
    ...
    BOOL    indUpscale_cropIn_horizLeft;
    BOOL    indUpscale_cropIn_horizRight;
    ...
    uint8_t lmCropAfterUpscale_left;    // v20 only
    uint8_t lmCropAfterUpscale_right;    // v20 only
    uint8_t lmCropAfterUpscale_top;        // v20 only
    uint8_t lmCropAfterUpscale_bottom;    // v20 only
    BOOL    lmCropOut_enable;            // v20 only
    uint16_t lmCropOut_horizStart;        // v20 only
    uint16_t lmCropOut_horizEnd;        // v20 only
    uint16_t lmCropOut_vertStart;        // v20 only
    uint16_t lmCropOut_vertEnd;            // v20 only
}
TFv10v20StripingOutputParams;
...
typedef struct
{
    ...
    RnrStripingOutputParams rnr_out_dn4;
    RnrStripingOutputParams rnr_out_dn8;
    RnrStripingOutputParams rnr_out_dn16;
    int16_t face_horizontal_offset;                                     // 16u
    int16_t face_vertical_offset;                                       // 16u

    uint16_t crop_stripe_to_image_boundary_dist_left;                    // 14u
    int16_t crop_output_stripe_width;                                    // 14u
    int16_t crop_frame_boundary_pix_align_right;                        // 1u

    int16_t post_crop_enable;                                            // 1u
    int16_t post_crop_first_pixel;                                        // 14u
    int16_t post_crop_last_pixel;                                        // 14u
    int16_t post_crop_first_line;                                        // 14u
    int16_t post_crop_last_line;                                        // 14u
}
...
typedef struct
{
    int32_t bx;
    int32_t by;
    uint32_t r_square_init;
    uint16_t r_square_shft;
    uint16_t r_square_scale;
}
abfOutStruct_v34;
...
typedef struct
{
    int luma_start_location_x;
    int chroma_start_location_x;

    int luma_scale_ratio_x;
    int chroma_scale_ratio_x;

    int luma_out_width;
    int chroma_out_width;

    int luma_input_width;
    int chroma_input_width;
}
DSXInStruct_V1D;  // HW parameters
...
typedef struct
{
    ...
    uint16_t upscale_dithering_mode_y;
    uint16_t upscale_dithering_mode_c;
}
...
typedef struct
{
    ...
    uint16_t y_dithering_mode;
    uint16_t c_dithering_mode;
    uint16_t c_rounding_option_h;
    uint16_t c_rounding_option_v;
}
...
PDPCv20v30stripingOutputParams;
typedef struct
{
    BOOL enable;
    int32_t pdaf_global_offset_x;
...
typedef struct
{
    BOOL binning_correct_enable;        //applied to HDR v30 + binning correct v10 combo only
    BOOL hdr_enable;                    //applied to HDR v30 only
    BOOL hdr_linear_mode;               // (0, 1), input -> wb -> inv wb -> output (alignment depends on hdr_lsb_align)
...
BayerFocusv24InputParam;

typedef struct
{
    uint64_t baf_roi_indexLUT[BF_STATS_RGNCOUNT_V25][2];
    uint16_t baf_h_scaler_en;
    uint16_t baf_fir_h1_en;
    uint16_t baf_iir_h1_en;
    uint16_t dummy_field_for_byte_padding;
...
BayerFocusv24OutputParam;

typedef struct
{
    uint64_t baf_roi_indexLUT[BF_STATS_RGNCOUNT_V23][2];
    MNScaleDownInStruct_V16_1D mndsParam;
    uint16_t enable;
    uint16_t dummy_field_for_byte_padding;
    uint16_t bank_sel;
}
BayerFocusv25OutputParam;
...
void pdaf_stripingSetup(const PDPCv20v30stripingInputParams* frameLevelParam, const int stripeStartX, const int stripeEndX, PDPCv20v30stripingOutputParams * pStripeLevelParam, const int32_t chip );
...
void LenrConfigPostCrop(const int16_t full_res_start, const int16_t full_res_end, const int16_t lenr_ds4_out_start, const int16_t lenr_ds4_out_end,
                        crop_1D *lenr_post_us4_crop);
int16_t getLenrDs4OutStart(const int16_t full_res_start, const int16_t ds4_fetch_start, const int16_t US4InitPhH,
                           const int16_t lenr_ds4_kernel_left, const BOOL edgeStripeLT, const int16_t mainOffsetInSource);
int16_t getLenrDs4OutEnd(const int16_t full_res_end, const int16_t ds4_fetch_end, const int16_t US4InitPhH,
                         const int16_t lenr_ds4_kernel_right, const int16_t lenr_ds4_dim, const BOOL edgeStripeRB, const int16_t mainOffsetInSource );
...
// TODO : rewrite this API
ifeBAFPartition decideBAFv23statsPartition(const int leftStripeValidEnd, const int rightStripeValidStart, const BayerFocusv23InputParam frameParam,
                                           const MNScaleDownInStruct_V16_1D mndsFrameParam, const BOOL is_fetch_given, const BOOL extend_for_baf);
...
void bayerFocusv24_IFEstripingSetup(const BayerFocusv24InputParam* frameParam,
                                    const MNScaleDownInStruct_V16_1D* mndsFrameParam,
                                    const int leftStripe,
                                    const ifeBAFPartition* ifePartition,
                                    const int stripeFetchStartX,
                                    const int stripeFetchEndX,
                                    BayerFocusv24OutputParam *stripeParam,
                                    WriteEnginePassParam* pWriteEnginePassParam,
                                    WriteEngineStripeParam* pWriteEngineStripeParam );
int updateRightFetch_BAFv24(const int originalFetchRightStart, const ifeBAFPartition* bafPartition,
                            const int pixelCrop, const BayerFocusv24InputParam* frameParam,
                            const MNScaleDownInStruct_V16_1D* mndsFrameParam );
int updateLeftFetch_BAFv24(const int originalFetchLeftEnd, const ifeBAFPartition* bafPartition,
                           const int pixelCrop, const BayerFocusv24InputParam* frameParam ,
                           const MNScaleDownInStruct_V16_1D* mndsFrameParam );
ifeBAFPartition decideBAFv24statsPartition(const int leftStripeValidEnd, const int rightStripeValidStart, const BayerFocusv24InputParam frameParam,
                                           const MNScaleDownInStruct_V16_1D mndsFrameParam, const BOOL is_fetch_given, const BOOL extend_for_baf);

void bayerFocusv25_IFEstripingSetup(const BayerFocusv25InputParam* frameParam,
                                    const MNScaleDownInStruct_V16_1D* mndsFrameParam,
                                    const int leftStripe,
                                    const ifeBAFPartition* ifePartition,
                                    const int stripeFetchStartX,
                                    const int stripeFetchEndX,
                                    BayerFocusv25OutputParam *stripeParam,
                                    WriteEnginePassParam* pWriteEnginePassParam,
                                    WriteEngineStripeParam* pWriteEngineStripeParam );
int updateLeftFetch_BAFv25(const int originalFetchLeftEnd,
                           const ifeBAFPartition* bafPartition,
                           const int pixelCrop,
                           const BayerFocusv25InputParam* frameParam ,
                           const MNScaleDownInStruct_V16_1D* mndsFrameParam);
int updateRightFetch_BAFv25(const int originalFetchRightStart,
                            const ifeBAFPartition* bafPartition,
                            const int pixelCrop,
                            const BayerFocusv25InputParam* frameParam,
                            const MNScaleDownInStruct_V16_1D* mndsFrameParam);
ifeBAFPartition decideBAFv25statsPartition(const int leftStripeValidEnd,
                                           const int rightStripeValidStart,
                                           const BayerFocusv25InputParam frameParam,
                                           const MNScaleDownInStruct_V16_1D mndsFrameParam,
                                           const BOOL is_fetch_given,
                                           const BOOL extend_for_baf);
...
void pdafStats_stripingSetup_2PD(const PDAFv20InputParam* frameLevelParam,
                             PDAFv20OutputParam* pStripeLevelParam,
                             const int leftStripe,
                             int32_t imgWidth);
void LCR_stripingSetup(const LCRv10InputParam* frameLevelParam,
                             LCRv10OutputParam* pStripeLevelParam,
                             const int leftStripe,
                             int32_t imgWidth);
void configure_square_parametersABF(int32_t width,
                        int32_t height, int16_t *r_square_scale, int16_t *r_square_shift);
