...
#include "wrsomeip_error_domains.h"
#include "wrapper/wrsomeip_payload_wrapper.h"
#include "wrapper/wrsomeip_message_wrapper.h"
...
private:
    ...
    {
        const types::byte_t* data = reinterpret_cast<const types::byte_t*>(&value_);
    ...
    void insertToHelper(Iterator out_iterator)
    {
        ...
        if (EndianChecker::isLittleEndianHost()) {
            types::byte_t intermediate_data[sizeof(value_)];
...
public:
     ...
     */
    explicit DeserializingEnumerator(std::shared_ptr<wrapper::WrsomeipPayload> payload)
     ...
     */
    DeserializingEnumerator(const types::byte_t* data, std::size_t size)
        : data_(data)
        , size_(size)
        , pos_(0)
    { }
...
private:
    const types::byte_t* data_;  ///< Array to deserialize from.
...
public:
     ...
     * \return
     */
    SizeCounter(const types::byte_t* data, std::size_t buf_size)
        : buf_size_(buf_size)
        , data_(data)
    { }
...
private:
    ...
    const types::byte_t* data_;  ///< Array to use as input for the deserialization of the length information.
};
...
public:
     ...
     */
    Deserializer(const types::byte_t* data, std::size_t size)
        : data_(data)
        , size_(size)
    {
        static_assert(IsSerializable<value_type>(), "No appropriate marshalling defined for this type!");
...
private:
    ...
    result_type getValueHelper(EnableIfBaseType<U>* = nullptr) const
    {
        ...
        if (EndianChecker::isLittleEndianHost()) {
            types::byte_t intermediate_data[sizeof(value_type)];
    ...
    result_type getValueHelper(EnableIfMembersAreTLV<U>* = nullptr) const
    {
        ...
        result_type result{};
        const types::byte_t* start = data_ + sizeof(length_field_type);
        const types::byte_t* end = start + len;
        const types::byte_t* current = start;
...
public:
     ...
     * \param size Size of data.
     */
    SequenceContainerDeserializer(const types::byte_t* data, std::size_t size)
        : data_(data)
        , len_()
        , size_(size)
    {
     ...
     */
    SequenceContainerDeserializer(const types::byte_t* data, std::size_t size, std::size_t len)
        : data_(data)
        , size_(size)
        , len_(len)
    { }
    ...
    result_type getValue() const
    {
        ...
        const types::byte_t* current_pos = data_ + sizeof(length_field_type);
        const types::byte_t* end_pos = current_pos + len_;
...
public:
     ...
     */
    AssociativeContainerDeserializer(const types::byte_t* data, std::size_t size)
        : data_(data)
        , size_(size)
        , len_()
    {
    ...
    result_type getValue() const
    {
        ...
        const types::byte_t* pos = data_;
        const types::byte_t* end_pos = data_ + len_;
...
private:
    const types::byte_t* data_;  ///< Pointer to the first data item.
...
/**
 * \brief Deserializes a range of types from a wrsomeip message.
 *
...
public:
     ...
     */
    Unmarshaller(std::shared_ptr<wrapper::WrsomeipMessage>& message)
...
private:
    ...
    std::shared_ptr<wrapper::WrsomeipPayload>
        payload_;  ///< Payload of the message that shall be used as the data source.
...
{
public:
    Unmarshaller(std::shared_ptr<wrapper::WrsomeipMessage>&)
    { }
...
/**
 * \brief Serializes a number of function arguments to the given wrsomeip payload object.
 *
